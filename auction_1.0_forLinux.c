//utiliza-se uma estrutura do tipo estatica em um ambiente UNIX
...
//***********************Definiçao das funçoes(prototipos/cabeçalho)***********************
...
void main(){
    ...
    int escolhaInt, idVendInt, idProdInt, idCompInt, valorMinInt, lanceInt, opInt ;
    char idComp[50], idProd[50], idVend[50], lance1[50] , produto[50], comprador[50], vendedor[50], valorMin[50], escolha[50], op[50];
    int i ; //auxiliar para condiçoes de paradas dos laços de repetiçao
    ...
    if( escolhaInt == 1 ){
        ...
        if (vazioProd() == FALSE && vazioComp() == FALSE){
            ...
            testeidProd(idProdInt);
            while( testeidProd(idProdInt) == FALSE){
            ...
            scanf("%s",op);
            verificarInt(op);
            while( verificarInt(op) == FALSE){
                printf("\nDigite a opçao com numeros naturais positivos\n>>>");
                scanf("%s",op);
                verificarInt(op);
                }
            opInt = atoi (op);
                ...
                do{
                    ...
                    opInt = atoi (op);
                        ...
                        if (ganhador == 0){
                            ...
                            printf("\nComprador %s com o codigo identificador %d fica com o produto %s por R$%d\n", nomeGanhador, ganhador,nomeProduto, valorMaior);
                            apagar(idProdInt);
                            printf("\nDigite qualquer tecla para voltar ao menu anterior\n>>>");
                            scanf("%d",&i);
                            if(i!='a') system("clear");
...
void lance(int lanceInt, int idProd, int idComp){
    ...
    if ( lanceInt > li.produto[i].valorMin ){
        ...
        strcpy(nomeProduto, li.produto[i].nome);
    }
    else
        printf("\nValor insuficiente!\n");
}
