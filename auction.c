...
int cheioVend(); //verifica se a vendedor para comprador esta cheia
int vazioVend(); //verifica se a vendedor para comprador esta vazia
...
void main(){
    ...
    if( escolhaInt == 1 ){
        ...
        while (testeidComp(idCompInt) == TRUE ){
            printf("\nCodigo identificador para comprador ja cadastrado!\n");
            printf("\nDigite outro codigo\n>>>"); scanf("%s",idComp);
            verificarInt(idComp);
            while( verificarInt(idComp) == FALSE){
                printf("\nDigite o codigo identificador com numeros naturais positivos\n");
